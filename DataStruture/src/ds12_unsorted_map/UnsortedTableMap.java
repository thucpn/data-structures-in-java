/*
 * UnsortedTableMap: 
 * 	-- Cái loại Map người ta nghĩ ra đầu tiên là UnsortedTableMap
 * 	-- Thèn này lưu trữ dữ liệu dựa trên ArrayList
 * 
 * Các thao tác:
 * 	O(1)	: size(), isEmpty()
 * 	O(n)	: get(), put(), remove(), entrySet(), keySet()
 * 
 * Nhận xét:
 * 	Các phương thức get(key), put(key, value), remove(key) muốn thực hiện được thì phải biết 
 * 	cái key đó nằm chỗ nào. Mà muốn vậy phải đùng hàm findIndex(key) lặp hết List để tìm
 * 	--> Chi phí cao 
 * 	
 */
