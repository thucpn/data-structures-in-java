/*
 * -- Mặc dù HashTable mang lại hiệu quả rất tốt nhưng các key của nó rất rời rạc và không 
 * 		có tương tác gì với nhau, nên người ta vẫn phát triển cấu trúc khác của Map. 
 * 		Đó là Sorted-Map. Mục đích chính là để phục vụ nhiều trường hợp cần sự tương tác
 * 		giữa 2 key với nhau.
 * 
 * -- Các key trong SortedTableMap được sắp xếp theo 1 thứ tự để tiện cho việc liên lạc
 * 
 * -- Các thao tác:
 * 		O(1)		: size(), firstEntry(), lastEntry()
 * 		O(logn)		: get(), ceilingEntry(), floorEntry(), lowerEntry(), higherEntry()
 * 		O(logn + s)	: subMap() - s là số entry cần report
 * 		O(n)		: put(), remove(), entrySet(), keySet()
 * 
 * -- Tốt:
 * 	+ Search - get() nhanh ổn định với Binary Search - logn
 * 	+ Tận dụng được sự tương tác giữa các entry cận kề. 
 * 	+ Phương thức submap() có nhiều ứng dụng thực tế nhưng chi phí ko cao
 * -- Xấu:
 * 	+ các thao tác put(), remove() chậm 
 * 
 * -- Ứng dụng: 
 * 	+ Xây dựng database các chuyến bay hỗ trợ việc truy vấn các chuyến bay từ x giờ tới y giờ
 * 		nhanh chóng
 * 	+ Gợi ý best choice với hàm floorEntry (Returns the entry with greatest key less than or equal
 * 		to given key) -> Ứng dụng gợi ý mua hàng xoá với giá nhỏ hơn giá nhập vào
 * 	+ SortedTableMap là tiền đề để xây dựng SkipList
 */
package ds14_sorted_map;
