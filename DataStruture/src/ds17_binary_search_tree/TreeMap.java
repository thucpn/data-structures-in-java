/*
 * 1. TreeMap: 
 * 	-- Là hiện thực hoá của cây nhị phân tìm kiếm dựa trên cấu trúc map
 * 	-- BalanceableBinaryTree: Cây nhị phân tìm kiếm cân bằng - cây AVL
 * 	-- 
 * 2. Các thao tác:
 * 	O(1)	: size(), isEmpty()
 * 	O(h)	: get(), put(), remove(), firstEntry(), lastEntry(),
 * 				ceilingEntry(), floorEntry(), lowerEntry(), higherEntry()
 * 	O(s+h)	: subMap 
 * 	O(n)	: entrySet(), keySet()
 * 	 
 * 3. Tốt:
 * 	-- vì get có thể áp dụng BinarySearchTree nên chi phí thấp, kéo theo đó là các thao tác
 * 		put và remove tại vị trí bất kì cũng là O(h)
 * 
 * 4. Xấu
 * 	-- Một số trường hợp cây có thể suy biến thành LinkedList
 * 	-- Do đó người ta đã tạo ra cây nhị phân tìm kiếm cân bằng ( tại mỗi nút, chiều cao của cây
 * 		con trái và cây con phải chênh nhau không quá 1)
 * 	-- Bằng cách xoay cây sau mỗi thao tác thêm, xoá, cây cân bằng đảm bảo hiệu suất cho các
 * 		thao tác đó là O(h), tránh trường hợp bị suy thoái
 * 5. Ứng dụng:
 * 	-- Ngoài hiệu suất cao, thì Cây AVL còn là tiền đề phát triển các cây khác như splay-tree, 
 * 		cây đỏ-đen, B-Tree
 */
package ds17_binary_search_tree;

import java.util.ArrayList;
import java.util.Comparator;
import ds08_general_tree.Position;
import ds09_binary_tree.LinkedBinaryTree;
import ds10_priority_queue.Entry;
import ds14_sorted_map.AbstractSortedMap;
