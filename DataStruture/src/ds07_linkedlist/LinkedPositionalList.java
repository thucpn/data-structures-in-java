/*
 * 1. PositionalList (LinkedList): 
 * 	-- Là hiện thực hoá của List dựa trên danh sách liên kết
 * 	-- Trong java Class java.util.LinkedList cũng tương tự như Class này
 * 	-- PositionalList được tạo ra nhằm khắc phục nhược điểm về kích thước của ArrayList,
 * 		giúp ta có nhiều giải pháp hơn để lựa chọn trong trường hợp thích hợp.
 * 	-- PositionalList (danh sách theo thứ tự) - từ "positional" là ám chỉ trong danh sách này
 * 		các thao tác sẽ được thực hiện theo nguyên tắc vị trí đầu, cuối, phía trước, kế tiếp.
 * 		Chính vì thế, hiện thực PositionalList bằng Doubly-Linked-List là thích hợp nhất
 * 2. Các thao tác:
 * 	-- O(1)		: size(), isEmpty(), first(), last(), 
 * 					set(), remove(), addFirst(), addLast(), addBefore(), addAfter()
 * 
 * 3. Tốt:	
 * 	-- So với DoublyLinkedList, PositionalList bổ sung thêm thao tác đến các Node kế cận
 * 		(addBefore(), addAfter())
 * 	-- So với ArrayList, PositionList không gặp vấn đề về kích thước
 * 
 * 4. Xấu:
 * 	-- Tuy chi phí của set(), remove(), addFirst(), addLast(), addBefore(), addAfter()
 * 		là O(1) nhưng đòi hỏi thêm 1 tham số truyền vào phải là tham chiếu Node. Điều này khá 
 * 		là rất khó khăn vì thực tế việc tìm thông tin của tham chiếu Node đó cũng tốn chi
 * 		phí O(n). 
 * 	-- Do đó việc thao tác của PositionalList vẫn chưa được gọi là linh động ( vì chỉ hiệu quả 
 * 		ở đầu hoặc ở những vị trí đã biết thông tin của các node Hàng xóm)
 * 
 * 5. Ứng dụng:
 * 	-- Nếu chương trình khắt khe về kích thước, thì PositionalList nên được sử dụng thay cho
 * 		ArrayList
 * 	-- Mặc dù ko linh động bằng ArrayList nhưng ở 1 số trường hợp chỉ có thao tác ở 2 đầu, hoặc
 * 		thao tác chủ yếu ở những Node mà thông tin hàng xóm đã có, sử dụng PositionalList là
 * 		điều tuyệt vời
 * 		
 */
package ds07_linkedlist;
